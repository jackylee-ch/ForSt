...
impl CompactionMerger {
    ...
    pub fn next_output_batch(&mut self) -> Result<Option<RecordBatch>> {
        ...
        while let Some(Reverse(entry)) = self.merge_heap.peek() {
            ...
            // 4b. SingleDelete 处理（conservative RocksDB semantics, 实现自 commit aa617121f）
            //
            // Contract: SingleDelete 仅在调用者保证该 key 自上次
            // delete-family op 以来最多 put 过一次时有效。违约行为退化为
            // 普通 Delete 语义（不会破坏数据）。
            //
            // Policy: 严格 `[SingleDelete, Put]`（exactly 2 versions）elide
            // — 两个 entry 都从本次 compaction 的输出中丢弃。其他 shape
            //（stacked SingleDeletes / intervening Merge / 额外 shadowed
            // Put）fallback 到普通 Delete 路径。
            //
            // 参见 `crates/forst-rs-engine/src/compaction.rs::emit_key_versions`
            // 的 `OpType::SingleDelete` 分支。
            if op == OpType::SingleDelete as u8 {
                // 精确 `[SingleDelete, Put]` 两元素：两个 entry 都丢弃。
                if versions.len() == 2
                    && versions[1].op_type == OpType::Put as u8
                {
                    self.advance_entry(); // drop SingleDelete
                    self.advance_entry(); // drop matching Put
                    last_emitted_key = Some(key.to_vec());
                    continue;
                }
                // Fallback: behave like Delete.
                if self.is_bottommost {
                    self.advance_entry();
                    last_emitted_key = Some(key.to_vec());
                    continue;
                }
                // 非 bottommost：保留 SingleDelete 标记（fallback 行为）
            }
