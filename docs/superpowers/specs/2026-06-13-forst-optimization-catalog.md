...
|---|---|---|
| **[#1] Concurrent SST-reader OPEN fan-out (Cycle-3)** | prefetch.rs `prime_opens_concurrent`; db.rs `prime_cold_reader_opens_concurrent`; FS `is_local()` trait gate | **SHIPPED 2026-06-14**, `FRS_SCAN_OPEN_FANOUT` (default-OFF, byte-identical). Mini-bench (real `DbImpl` scan, RTT=2ms sim-50G/s, reader-cache-cold): K4 **2.58×** / K8 **2.67×** / K16 **3.05×** open-fanout; composed with cold-prime K4 **3.82×** / K8 **4.01×** / K16 **5.25×**. 7 engine ITs + 3 prefetch UTs |
