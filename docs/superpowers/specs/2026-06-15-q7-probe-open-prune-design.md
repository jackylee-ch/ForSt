...
**Status:** SHIPPED — engine implemented (§3.4 side-cache, flag
`FRS_RS_PROBE_BLOOM_PRUNE`, default-OFF); post-build mini-bench CONFIRMS `mr1_on`
realizes the `cold_hit` ceiling (§5.1). Byte-identity TDD green. Awaiting the
remote q7/q9/q20 @100M A/B.

**Implementation (branch `mr1-impl`):**
- `crates/forst-rs-storage/src/sst/reader.rs` — `SstReaderImpl::prefix_bloom()`
  accessor (borrow the decoded v3 prefix bloom).
- `crates/forst-rs-storage/src/sst/mod.rs` — re-export `PREFIX_BLOOM_LEN`.
- `crates/forst-rs-engine/src/db.rs`:
  - `probe_bloom_prune_enabled()` (flag helper, live-read).
  - `DbImpl.prefix_bloom_meta_cache: ArcSwap<HashMap<FileNumber, Option<Arc<Sbbf>>>>`
    (the §3.4 side-cache).
  - `cache_prefix_bloom_meta()` + the cache-HIT and post-open population in
    `get_or_open_sst_reader` (sticky bloom survives reader eviction).
  - `probe_bloom_meta_negative()` (decode-free, open-free prune predicate).
  - the pre-open prune + open-fanout filter in `build_lazy_prefix_key_stream_sel`.
- Byte-identity + skip-proof + no-false-negative TDD:
  `test_mr1_probe_bloom_prune_byte_identical_and_skips_cold_opens`,
  `test_mr1_probe_bloom_prune_no_false_negative_full_keyspace` (db.rs tests).
- Mini-bench: `crates/forst-rs-bench/benches/probe_open_bloom_prune.rs`
  (`mr1_on` arm added).
...
- **Post-build (DONE):** the 4th arm `mr1_on` (flag ON, full `cold_all` fixture,
  bloom-meta cache primed once then readers evicted per iter — the q7 cold-probe
  regime) realizes the ceiling. See §5.1.

### 5.1 Post-build result — `mr1_on` REALIZES the ceiling (criterion median, in-memory FS, `--measurement-time 3`, 2026-06-15)

| SSTs | `cold_all` (OFF) | `cold_hit` (ceiling) | **`mr1_on`** | `warm_all` | **MR-1 win (cold_all/mr1_on)** |
|---:|---:|---:|---:|---:|---:|
| 8   | 8.01 µs  | 2.85 µs | 3.30 µs | 2.20 µs  | 2.4× |
| 32  | 29.44 µs | 2.80 µs | 4.15 µs | 5.36 µs  | **7.1×** |
| 64  | 59.74 µs | 2.89 µs | 5.55 µs | 9.68 µs  | **10.8×** |
| 128 | 130.0 µs | 2.89 µs | 7.92 µs | 18.11 µs | **16.4×** |

Readings:
1. **`mr1_on` tracks `cold_hit`, NOT `cold_all`** — it is near-flat (3.3 → 7.9 µs
   over 8 → 128 SSTs) while `cold_all` blows up linearly (8 → 130 µs). The prune
   fires: only the `present=2` bloom-positive SSTs are re-opened cold; the
   bloom-negative residue is skipped from the RAM-resident meta cache with no open.
2. **`mr1_on` BEATS `warm_all` at high N** (7.9 vs 18.1 µs at 128): warming still
   merges all 128 sources; MR-1 prunes the merge fan-out to 2 — the free compound
   with S2 / Approach 1.
3. The small `mr1_on − cold_hit` gap (e.g. 7.9 vs 2.9 µs at 128) is the meta-cache
   `load()` + `check_hash` over all 128 metas per probe (an O(N) RAM pass, no
   I/O) — negligible vs the cold opens it replaces, and bounded further by
   Approach 1 (fewer metas to scan once the source count is leveled).
4. **No shallow tax** (ssts_8: mr1_on 3.30 vs cold_all 8.0 µs — already a win, and
   well below the cold_all slope) — uniform-config safe.

**Gate verdict (post-build): PASS.** The lever realizes the measured ceiling.
Next is the remote q7/q9/q20 @100M A/B with `FRS_RS_PROBE_BLOOM_PRUNE=1` (the
binding numbers; NOT run here — a uniform-config sweep is live on the box).

The skip is also pinned by a UNIT test (not just the bench): on a 16-SST deep
fixture, a post-evict pruned probe re-opens EXACTLY the 2 bloom-positive SSTs
(`opened == present`), and the emitted rows are byte-identical to OFF.
