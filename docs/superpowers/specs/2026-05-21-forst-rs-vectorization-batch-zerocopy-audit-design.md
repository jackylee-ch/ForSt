...
| **V3** | `state/ForStRsAsyncListStateV2.java:135-159` (`asyncAdd` → full-PUT) — **CLOSED 2026-05-22 (V20 sub-spec)**. Shipped as Format B (multi-chunk) decoder + classifier APPEND_MERGE routing in commit `11d7e0dfaf0` on `forst-rs-jdk25`. Q19 relief = 10.2 s (under-delivered vs estimated 30 s; ablation trigger fired but direction favorable — investigation deferred to Phase C MapState V2 work since `UpdatableTopNFunction` uses both ListState AND MapState). | T1 | batch | **CRIT** | Q19 | Q19 (135 → 124.85 s; rocksdb ratio 1.08× → **1.16×**) | 10 (empirical) | HIGH (measured) | 10.0 |
...
| **V20** | `state/ForStRsAsyncListStateV2` (Flink-integrated) + `state/ForStRsListStateV2` (standalone) — **CLOSED 2026-05-22** via sub-spec `2026-05-22-v20-liststate-format-unification-design.md`. Resolution: Format B (multi-chunk) decoder adopted by both classes; no snapshot migration needed (legacy single-chunk subsumed). V3 unblocked and shipped. | T1 | partial wiring (cross-tier) | HIGH | Q19 (gates V3) | n/a (closed) | n/a (closed) | HIGH | n/a |
...
| **D4** | 25 → empirical: see note below | Arithmetic: per-row FFM ≈ 1 µs × Q19's ~3 LIST_ADDs per event × 100 M events / N=1024 batch size ≈ 300 ms FFM overhead after V3 (negligible). Real relief from batching = the per-row Arena.ofConfined() allocation (`Arena` alloc ≈ 50 ns × 300 M rows = 15 s) plus per-row memcpy cost (~30 ns × 300 M = 9 s) = ~24 s. **Empirical 2026-05-21**: cargo criterion-style timing of `merge_append_batch/1024` measured median **672 µs / 1024 = 656 ns/row** under release mode (includes engine RocksDB get+combine+put, not just FFM). Original "≤ 10 µs/batch" target was FFM-only and unrealistic; revised target ≤ 1 µs/row achieved. | MED | empirical timing |
...
|---|---|---:|---|
| A.1 (V4 infra only, 2026-05-21) | D4 (25) | 0 s — V4 is zero-behavior-change for current callers (LIST_ADD still routes through PUT). Predicted: ALL Nexmark queries stay within v3.8 baseline ± 2%. | If any Nexmark query deviates > 5% from v3.8 baseline → HALT (V4 touched an unexpected path). **2026-05-21 attribution PASS** (commit `3e1d5a1c2a9`): median-of-3 measurements with V4 jar deployed — Q5 36.34s (+2.9% vs v3.8), Q11 72.63s (-1.3%), Q12 29.21s (-3.3%), Q15 19.31s (+5.3%, borderline single-sample), Q19 134.98s (-0.04%), Q20 54.90s (-0.4%), Q23 54.74s (+0.2%). First-sample Q5/Q11 outliers (+14%/+8%) confirmed as JIT warmup / thermal noise via pre-V4 control bench (Q5 36.86s mean, Q11 73.60s mean on pre-V4 jar). |
| A.2 (V3, shipped 2026-05-22) | D3 re-derived 30 s | **Measured 10.2 s on Q19** | Triggered (10.2 outside [21, 39]) — investigation: Q19's UpdatableTopNFunction uses both ListState AND MapState V2; V3 only addresses ListState. Remaining Q19 headroom is on Phase C (MapState V2 off-heap, V2 violation). Direction favorable — no revert. |
