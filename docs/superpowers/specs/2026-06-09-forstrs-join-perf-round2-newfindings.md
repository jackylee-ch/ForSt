...
> **Codebases:** Flink `/Users/lijunqing/Code/stczwd/flink` (HEAD `01f2064dfde`); ForSt-C++ = this repo `main` (`git show main:<path>`); forst-rs = this repo `forst-rs` HEAD.
>
> **Round-3 (2026-06-10) — re-verification + prioritization.** A second 5×10 workflow (5 perspectives × 10 rounds re-score, per-item file:line re-verification, adversarial-verify on every High candidate, synthesize) re-checked all 24 items on current code and assigned **Priority = High/Medium/Low** (filled below; previously blank). Outcome: **6 High, 15 Medium, 3 Low.** One material correction: **OPT-N22's "key-AGNOSTIC routing" claim was refuted** — the live router is key-group-affine (`Math.floorMod(keyGroup, N)`); `leaseWorker` is dead code (see OPT-N22, rewritten). The adversarial pass also demoted the operator-side short-circuit items (OPT-N01/N02) from the implied "headline" to Medium, because q7/q9/q20 are **inner** joins that emit the full cross-product (no short-circuit applies). Status still blank by design.
...
| `RoutingStateExecutor` is "gated-off scaffolding"; key-group-affine routing is "a spec" | `RoutingStateExecutor` is **fully built AND key-group-affine** — routes by `Math.floorMod(keyGroup, N)` (`offer:346-352`, `executeRequestSync:305-306`); `leaseWorker` is **dead code**. Still opt-in (`FRS_RS_PARALLEL_EXECUTOR=1`). **Round-3 correction:** this doc's own Round-2 "key-AGNOSTIC" claim was also wrong — see OPT-N22. | #74 #85 + R3 verify |
...
|---|---|---|---|---|---|---|---|
| [OPT-N10](#opt-n10) | Retraction batches collapse to per-row sync FFI via requiresOrderedDispatch | NEW | Vectorization | q7/q9/q20 | high | **High** | |
| [OPT-N12](#opt-n12) | Memtable prefix-scan rebuilds 16-shard locked snapshot+sort+Arc-copy per probe | NEW | CPU/read | q9/q20 | high | **High** | |
| [OPT-N14](#opt-n14) | batch-GET L0 has no newest-first short-circuit; per-probe L0 re-sort + linear find_sst | OVERTURNS | LSM/read-amp | q4/q9/q20 | high | **High** | |
| [OPT-N16](#opt-n16) | Engine point-get to_vec() at every tier; borrowed primitives + batch_get_arrow unwired | NEW | Zero-copy | q7/q9/q20 | high | **High** | |
| [OPT-N18](#opt-n18) | Value-carrying merge BYPASSED for memtable-resident keys → get_internal re-walk + copy | OVERTURNS | Read-amp | q4/q9/q20 | high | **High** | |
| [OPT-N24](#opt-n24) | Merge-stored state de-vectorizes batch GET into N serial get_internal walks | NEW | Vectorization | q9/q20 | high | **High** | |
| [OPT-N01](#opt-n01) | Join fully materializes other-side into on-heap ArrayList before first match (no lazy/short-circuit) | NEW | Batch/Operator | q7/q9/q20 | high | Medium | |
| [OPT-N02](#opt-n02) | 128-entry iterator soft-cap → ceil(N/128) serial AEC re-dispatches per hot key | NEW | Iterator-lifecycle | q9/q20 | high | Medium | |
| [OPT-N03](#opt-n03) | AbstractStateIterator.onNext O(N²/128) ArrayList result-copy on continuation | REFINES | Iterator-lifecycle | q7/q9/q20 | high | Medium | |
| [OPT-N04](#opt-n04) | No-UK join state = per-record asyncGet→asyncPut RMW; no engine merge/add | NEW | Operator/Write | q9/q20 | high | Medium | |
| [OPT-N05](#opt-n05) | No predicate pushdown — JoinCondition applied row-by-row in Java after full decode | NEW | Vectorization | q7/q9/q20 | high | Medium | |
| [OPT-N07](#opt-n07) | ValueState join side (q7) has NO read cache and NO write-staging buffer | NEW | Batch/Cache | q7 | high | Medium | |
| [OPT-N08](#opt-n08) | getIterPrefix force-drains write buffer per probe; checkpoint force-drains all MapStates | NEW | Batch/Checkpoint | q7/q9/q20 | high | Medium | |
| [OPT-N09](#opt-n09) | statebuf forEachEntry linearly rescans ALL liveRows (whole-state) per probe | NEW | Memory/scan | q9/q20 | high | Medium | |
| [OPT-N11](#opt-n11) | Active-memtable point GET = BTreeMap descent + shard RwLock + key-copy bound (not O(1)) | REFINES | CPU/read | q4/q9/q20 | high | Medium | |
| [OPT-N13](#opt-n13) | k-way merge does 2× O(k) linear peek per emitted key; BinaryHeap deferred | NEW | CPU/read | q9/q20 | high | Medium | |
| [OPT-N15](#opt-n15) | KV-block (default) walk_key reconstructs ≤15 keys + re-parse per probe on cache hit | REFINES | CPU/read | q4/q9/q20 | high | Medium | |
| [OPT-N17](#opt-n17) | Iterator buffers Arc::from(key)+Arc::from(value) per row even on cache hit | REFINES | Zero-copy | q9/q20 | high | Medium | |
| [OPT-N19](#opt-n19) | Iterator wire format row-major AoS (GET is columnar); full key incl shared prefix re-copied | NEW | Arrow/Columnar | q9/q20 | high | Medium | |
| [OPT-N22](#opt-n22) | RoutingStateExecutor IS key-group-affine (floorMod) + opt-in; leaseWorker is dead code; q8-revert cause OPEN | OVERTURNS | Scheduling | q7/q9/q20 | high | Medium | |
| [OPT-N23](#opt-n23) | Read pool hard-clamped to 4 threads, process-global shared vs ForSt per-executor | REFINES | Scheduling | q9/q20 | high | Medium | |
| [OPT-N06](#opt-n06) | No-UK probe replays each record `cnt` times → O(multiplicity) OuterRecord allocs | NEW | CPU/alloc | q20 | high | Low | |
| [OPT-N20](#opt-n20) | SST bloom = scalar 8-probe loop + re-hash per L0 file + 256-bit unaligned blocks | NEW | CPU/SIMD | q9/q20 | high | Low | |
| [OPT-N21](#opt-n21) | Parallel-iter path WIRED but gated-off, BUILD-only, unreachable from sync values()/entries() | OVERTURNS | Vectorization | q7/q9/q20 | high | Low | |
...
- **Dimension**: Batch Execution / Operator state-access · **Queries**: q7/q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Iterator lifecycle · **Queries**: q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Iterator lifecycle / copy-count · **Queries**: q7/q9/q20 (UK-side variants) · **Tag**: REFINES · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Operator fusion / Write path · **Queries**: q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Vectorization / predicate pushdown · **Queries**: q7/q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: CPU / allocation amplification · **Queries**: q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Low · **Status**:
...
- **Dimension**: Batch / Cache asymmetry · **Queries**: q7 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Batch execution / Checkpoint × join · **Queries**: q7/q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Memory / scan complexity · **Queries**: q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Vectorization / ordered-replay amplification · **Queries**: q7/q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: High · **Status**:
...
- **Dimension**: CPU / read path · **Queries**: q4/q9/q20 · **Tag**: REFINES · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: CPU / read path (iterator) · **Queries**: q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: High · **Status**:
...
- **Dimension**: CPU / read path (merge) · **Queries**: q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: LSM read-amplification · **Queries**: q4/q9/q20 · **Tag**: OVERTURNS (v1 "read is cheap") · **Confidence**: high (FACT) · **Priority**: High · **Status**:
...
- **Dimension**: CPU / read path (SST block decode) · **Queries**: q4/q9/q20 · **Tag**: REFINES · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Zero-copy · **Queries**: q7/q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: High · **Status**:
...
- **Dimension**: Zero-copy (iterator path) · **Queries**: q9/q20 · **Tag**: REFINES · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Read-amplification · **Queries**: q4/q9/q20 · **Tag**: OVERTURNS (the "q4 2× read-path fix is general") · **Confidence**: high (FACT) · **Priority**: High · **Status**:
...
- **Dimension**: Arrow / Columnar · **Queries**: q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: CPU / SIMD · **Queries**: q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: Low · **Status**:
...
- **Dimension**: Vectorization / parallelism · **Queries**: q7/q9/q20 · **Tag**: OVERTURNS (v1 "frsVecIterPrefixOpenBatchParallel UNUSED") · **Confidence**: high (FACT) · **Priority**: Low · **Status**:
...
### <a id="opt-n22"></a>[OPT-N22] RoutingStateExecutor IS key-group-affine (floorMod) + opt-in; `leaseWorker` is dead code; the q8-revert cause is OPEN

> **CORRECTED in Round-3 verification.** The original Round-2 claim ("routing is key-AGNOSTIC `leaseWorker`, the affine fix is docs-only") is **refuted by current code**. The live routing path is key-group-affine.

- **Dimension**: Scheduling / thread-affinity · **Queries**: q7/q9/q20 · **Tag**: OVERTURNS (corrects this doc's own Round-2 claim, not just v1) · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:

#### Symptom & Impact
`RoutingStateExecutor` is a **complete N-worker offloading executor** with a real `fullyLoaded()` and a depth>1 path, wired behind `FRS_RS_PARALLEL_EXECUTOR=1` (opt-in; default is still the inline depth-1 `VectorizedExecutor`). **The routing IS key-group-affine:** both the async batch path (`RoutingRequestContainer.offer`) and the sync path (`executeRequestSync`) compute `Math.floorMod(keyGroup, workers.length)` and route to `workers[w]`, so a given key-group's state always lands on **one** worker (comment at `:174` states exactly this). The `leaseWorker()`/`releaseWorker()` free-worker methods are **defined but UNUSED** — they are dead code, and the Round-2 finding mistook them for the live path. So the per-worker `MapStateCache` is **not** fragmented across workers by routing.

This leaves a real, unresolved problem: Flink HEAD `e2aa17ade11` **did** revert OPT-01 default-on for a measured **q8 windowed-join ~40% under-emit** (3,010,888 → 1,819,576). Since routing is affine, **the under-emit mechanism is NOT key-agnostic fragmentation — its root cause is OPEN** (candidates: per-worker cache coherence under namespace/window ordering, completion-ordering across workers, or the known nondeterministic q8 race noted in project memory).

#### Affected Code Paths
- Affine routing (live): `RoutingStateExecutor.java:305-306` (`int kg = ctx.getKeyGroup(); int id = Math.floorMod(kg, workers.length)` in `executeRequestSync`), `:346-352` (`RoutingRequestContainer.offer` → `int w = Math.floorMod(kg, workers.length); subs[w].offer(request)`), `:174` (comment: "per-worker sub-container by keyGroup % N, so a given key-group's state always lands in ONE [worker]"), `:212/238/285` (per-worker `workers[id].executeBatchRequests`).
- Dead code: `RoutingStateExecutor.java:407` (`leaseWorker`), `:424` (`releaseWorker`) — unreferenced by the offer/execute paths.
- Wiring + revert: `ForStRsAsyncKeyedStateBackend.java:1149-1160` (returns `VectorizedExecutor` unless `FRS_RS_PARALLEL_EXECUTOR=1`), `:1142-1148` (q8 under-emit rationale); Flink HEAD `e2aa17ade11`.

#### Root Cause
FACT (routing): the executor routes by `floorMod(keyGroup, N)` — key-group-affine. The Round-2 "key-agnostic" claim is **wrong** (it read the unused `leaseWorker`). HYP (open): the q8 under-emit that gates default-on is **not** explained by routing; its cause is undetermined and must be root-caused before any default-on.

#### Proposed Optimization
The affine router already exists, so the lever is **(1) root-cause the q8 windowed-join under-emit** (build the deterministic q8 repro first — per project memory the race is nondeterministic), **(2) fix it** (likely per-worker cache coherence / completion ordering, NOT routing), then **(3) default-on the depth>1 executor** to remove the inline-mailbox serialization the v1 thesis identifies. Optionally delete the dead `leaseWorker`/`releaseWorker` to prevent future mis-reads.

#### Expected Impact
A correct, default-on depth>1 executor remains the **largest single throughput lever** per the v1-confirmed dispatch thesis — but it is **blocked on an open correctness bug**, not on a missing routing feature. That gap (high effort + correctness-critical + unknown root cause) is why this is Medium, not High, despite the impact ceiling.

#### Complexity & Risks
High + correctness-critical. The blocker is a real data-loss bug (q8 under-emit) whose mechanism is now unknown (the previously-assumed cause is refuted). Needs a deterministic repro before any change.

#### Validation Plan
Build the deterministic q8 repro; instrument per-worker cache + completion ordering to locate the actual under-emit cause; verify the fix restores exact emit count; only then A/B q7/q9/q20 wall with depth>1 default-on.

#### Dependencies
Gated on the q8 deterministic repro + root-cause. Relates to OPT-N21 (drain parallelism), OPT-N23 (read-pool sizing).

---

### <a id="opt-n23"></a>[OPT-N23] Read pool hard-clamped to 4 threads, process-global shared vs ForSt per-executor

- **Dimension**: Scheduling · **Queries**: q9/q20 · **Tag**: REFINES · **Confidence**: high (FACT) · **Priority**: Medium · **Status**:
...
- **Dimension**: Vectorization / batch-shatter · **Queries**: q9/q20 · **Tag**: NEW · **Confidence**: high (FACT) · **Priority**: High · **Status**:
...
- The **highest-leverage path** per Round-3 prioritization (5×10 re-score + adversarial verify):
  - **The 6 High items are immediately actionable, mostly engine-side reads:** **OPT-N16** (wire the existing `batch_get_arrow`), **OPT-N12** (drop the redundant per-probe sort + snapshot reuse), **OPT-N18** (carry value from the memtable cursor instead of `get_internal` re-walk), **OPT-N14** (port the L0 short-circuit into the batch path), **OPT-N24** (keep merge-typed join state vectorized), and **OPT-N10** (don't collapse retraction batches to per-row FFI). N16/N12 are the lowest-risk wins; N14/N24 carry the most read-amp.
  - **OPT-N22 (default-on the depth>1 executor) is the largest *impact ceiling*** per the v1-confirmed dispatch thesis, **but it is Medium, not High** — the affine router already exists (Round-3 correction), so the work is root-causing+fixing the q8 windowed-join under-emit that gates default-on (an open correctness bug), which is high-effort and uncertain.
  - **Operator-side net-new (OPT-N01/N02/N03/N04)** are Medium: real, but the adversarial pass showed the headline short-circuit/continuation wins are bounded for the q7/q9/q20 **inner** joins (full cross-product is emitted; the gain is container/GC and large-bucket tails, not decode/emit).
  - **Read/CPU micro-ops (OPT-N11/13/15/20) and bounded items stay Medium/Low** — the ~10-25% envelope, not the headline.
