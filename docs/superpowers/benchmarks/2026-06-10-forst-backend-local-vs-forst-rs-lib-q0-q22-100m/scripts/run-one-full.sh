...
JFR_JAVA_OPTS=""
if [ "${JFR_ENABLED:-0}" = "1" ]; then
  JFR_DIR="$BENCH_DIR/diagnostics/$RUN_ID"
  mkdir -p "$JFR_DIR"
  JFR_FILE="$JFR_DIR/${JFR_FILE_NAME:-taskmanager.jfr}"
  JFR_JAVA_OPTS="-XX:StartFlightRecording=name=${JFR_NAME:-$RUN_ID},settings=${JFR_SETTINGS:-profile},delay=${JFR_DELAY:-20s},duration=${JFR_DURATION:-${MAXSEC}s},filename=$JFR_FILE,dumponexit=true"
fi
...
if [ "${INSTALL_FLINK_FORST_BACKEND_JAR:-1}" = "1" ]; then
  FLINK_FORST_BACKEND_JAR="${FLINK_FORST_BACKEND_JAR:-$BENCH_DIR/lib/flink-statebackend-forst-2.2.1-mapcache.jar}"
  if [ -f "$FLINK_FORST_BACKEND_JAR" ]; then
    echo "=== install patched flink-statebackend-forst jar ==="
    rm -f "$FLINK_HOME"/lib/flink-statebackend-forst-*.jar
    cp "$FLINK_FORST_BACKEND_JAR" "$FLINK_HOME"/lib/flink-statebackend-forst-2.2.1.jar
    sha256sum "$FLINK_HOME"/lib/flink-statebackend-forst-2.2.1.jar
  else
    echo "WARN: patched flink-statebackend-forst jar not found: $FLINK_FORST_BACKEND_JAR"
  fi
else
  echo "=== keep image flink-statebackend-forst jar ==="
  ls -lh "$FLINK_HOME"/lib/flink-statebackend-forst-*.jar
  sha256sum "$FLINK_HOME"/lib/flink-statebackend-forst-*.jar
fi
...
echo "JFR_JAVA_OPTS=${JFR_JAVA_OPTS:-}"
...
mkdir -p "$RUN_DIR/primary" "$RUN_DIR/checkpoints" "$BENCH_DIR/tmp/$RUN_ID/io" "$BENCH_DIR/tmp/$RUN_ID/java" "$LOG_DIR" "$BENCH_DIR/diagnostics/$RUN_ID"
...
state.backend.forst.map-state.iterator-cache-size: ${FORST_MAP_STATE_ITERATOR_CACHE_SIZE:-4096}
"
fi
...
env.java.opts.taskmanager: "$JFR_JAVA_OPTS"
