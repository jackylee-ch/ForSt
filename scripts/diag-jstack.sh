...
echo "cluster up; submitting $QUERY"
JAVA_HOME="$JDK25" "$NEXMARK_HOME"/bin/run_query.sh oa "${QUERY}" > /tmp/diag-q5js.out 2>&1 &
...
echo "=== RUNNABLE task threads + first forstrs/timer/join frame (dump #1) ==="
